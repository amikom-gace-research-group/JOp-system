...
class Agent():
    ...
    def train(self):
        ...
        while self.round_num < configs.TOTAL_EPSIODES:
            ...
            new_state_value = self.RL.update_sarsa(self.current_state, choosen_action, new_state, choosen_action, reward)
...
def main():
                ...
                for pi, pmode in enumerate(PMODE_ID.keys()):
                    if pmode in agent.last_cl.keys():
                        if agent.round_num == configs.TOTAL_EPSIODES and not (agent.RL.prohibited_states[pi][agent.last_cl[pmode][0]] if agent.last_cl[pmode][0] in agent.RL.prohibited_states[pi].keys() else False):
                            ...
                            with open('results/result_sarsa.csv', 'w', newline='') as output_file:
                            ...
                            if oracle_res:
                                with open('results/oracle_sarsa.csv', 'w', newline='') as output_file:
    ...
    df_reward.to_csv("results/rewards_sarsa.csv", index=False)
