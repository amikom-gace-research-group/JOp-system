...
from configs import EPISLON, MIN_EPSILON, RUNTIME_GUARD, PMODE_ID,\
                    LOGGING_LEVEL, MODELS, ALPHAS, CONSTRAINTS, SPEC_ORDER, WARNING
...
print("Using Policy Gradient REINFORCE Algorithm")
...
class Agent():
    def __init__(self, board: IBoard):
        ...
        self.RL = REINFORCE(len(self.states[0]), len(Action))
        self.cl_index = 0
    ...
    def train(self):
        ...
        while self.round_num < configs.TOTAL_EPSIODES:
            ...
            for _ in range(len(self.states)):
                ...
                reaction = self.RL.get_action(self.current_state, self.epsilon[self.pmode_index])
                if reaction:
                    choosen_action, log_prob = reaction
                else:
                    continue
            ...
            self.RL.update(log_probs, rewards)
...
def main():
                ...
                for pi, pmode in enumerate(PMODE_ID.keys()):
                    if pmode in agent.last_cl.keys():
                        if agent.round_num == configs.TOTAL_EPSIODES and not (agent.RL.prohibited_states[pi][agent.last_cl[pmode][0]] if agent.last_cl[pmode][0] in agent.RL.prohibited_states[pi].keys() else False):
                            ...
                            with open('results/result_reinforce.csv', 'w', newline='') as output_file:
                            ...
                            if oracle_res:
                                with open('results/oracle_reinforce.csv', 'w', newline='') as output_file:
    ...
    df_reward.to_csv("results/rewards_reinforce.csv", index=False)
